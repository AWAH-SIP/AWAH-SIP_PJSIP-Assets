...
if [ "$1" = "build" ]; then
	...
	if [ -z "$(ls -A $APP_SRCDIR)" ]; then
		...
		cd $APP_SRCDIR
        sed -i -e '/ifneq ($(findstring sse2,@ac_webrtc_instset@),)/,+7d' third_party/build/os-auto.mak.in
        sed -i -e '/else ifneq ($(findstring mips,@ac_webrtc_instset@),)/,+16d' third_party/build/os-auto.mak.in
